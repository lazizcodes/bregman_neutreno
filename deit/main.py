...
def main(args):
    ...
    # print(is_rank0)
    # assert 1==2
    name = f'{current_time}_{args.model}' + args.output_dir.split('/')[-1]
    ...
    def add_pi_mask():
        for i in range(12):
            model_without_ddp.blocks[i].attn.register_buffer(f'pi_mask', torch.ones(1, 4,197,197))
            model_without_ddp.blocks[i].attn.pi_j = torch.nn.Parameter(torch.ones(1, 4,1,197), requires_grad = False)
            model_without_ddp.blocks[i].attn.register_buffer(f'pi_j_mask', torch.ones(1, 4,1,197))
    if args.resume:
