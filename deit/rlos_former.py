...
class Attention(nn.Module):
    ...
    def forward(self, x, v0 = None):
        ...
        ############### Cosine similarity between attention matrices
        attn_ = torch.flatten(attn, start_dim = 2)
        attn_norm = torch.norm(attn_, 2, dim = -1, keepdim= True)
        attn_ = attn_/attn_norm
        attn_cossim = torch.tril(attn_ @ attn_.transpose(-2, -1), diagonal = -1).sum(dim = (-1, -2))/3.
        attn_cossim = attn_cossim.mean()
...
class Block(nn.Module):
    ...
    def forward(self, x, v0 = None):
        if self.layerth == 0:
            ...
            x = x + self.drop_path(self.attn(self.norm1(x), v0))
