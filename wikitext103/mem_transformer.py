...
class RCudaFastWeightSumLinearTransformerLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0,
                 pre_lnorm=False, eps=1e-5, layer_id=None, num_layer=None,
                 skip_attn_normalization=False, use_sum_norm=False, layerth = None):
        super(RCudaFastWeightSumLinearTransformerLayer, self).__init__()
        print(f"Using RCudaFastWeightSumLinearTransformerLayer {layer_id} -")
    ...
    def forward(self, h, attn_mask=None, mems=None,
                carry_over_fast_weight=False, head_v0 = None):
        # multihead attention
        # shape h: (len, B, n_head * d_head)
        ...
        ############ R-linear-former
        ### only for self-attention since qlen must = klen 
        src_v = 0.2*(head_v0  - head_v) if head_v0 is not None else 0.
        # import pdb;pdb.set_trace()
        layer_out = layer_out + src_v
        ############ 
...
class GMMAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
                 pre_lnorm=False, update_mode='not_vector'):
        ...
        self.update_mode = update_mode
        if self.update_mode is not 'vector':
            # assert 1==2
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        if self.update_mode is not 'vector':
            # print(self.pi.shape)
            # assert 1==2
            attn_prob = self.pi_mask[:h.size(0),:c.size(0),:,:] * torch.exp(QK_distance0)

            # attn_prob = torch.clamp(torch.square(self.pi), max = 1.)[:h.size(0),:c.size(0),:,:] * torch.exp(QK_distance0) # qlen x klen x bsz x n_head
        else:
            assert 1==2
            attn_prob = torch.clamp(torch.abs(self.pi), max = 1.)[:h.size(0),:c.size(0),:,:] * torch.exp(QK_distance0) # qlen x klen x bsz x n_head
...
class MGKAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
                 pre_lnorm=False, update_mode='rbf'):
...
class Con1DAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
                 pre_lnorm=False):
        super(Con1DAttn, self).__init__()
        ...
        self.pre_lnorm = pre_lnorm
        self.kernel_size = 5
        self.conv_weight = nn.Parameter(torch.ones(self.n_head, 1,self.kernel_size, 1), requires_grad = True)
        # self.conv_weight = nn.Parameter(torch.ones(self.n_head, 1,self.kernel_size, 1)/3., requires_grad = True)
        self.conv_weight_mask = torch.eye(self.kernel_size)
        self.conv_weight_mask[-2:, -2:] = 0
        self.conv_weight_mask = self.conv_weight_mask[None, None, :, :]
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        mask = (torch.tril(torch.ones(attn.shape[-2], attn.shape[-1]))/3.).to(head_q.device)
        mask[:, 0] = 1.
        mask[:, 1] = 1/2.

        for i in range(self.n_head):
            attn[:, i, ...] = F.conv2d(attn_score[:, i, ...].unsqueeze(1), (self.conv_weight*self.conv_weight_mask.to(head_q.device))[i,...].unsqueeze(0), padding = 'same').squeeze(1)
        attn_score = (attn*mask).permute(2,3,0,1)
...
class MultiHeadAttn(nn.Module):
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        x_norm = torch.norm(attn_out.permute(1,0,2), 2, dim = -1, keepdim= True)
        x_ = attn_out.permute(1,0,2)/x_norm
        # x_cossim = torch.einsum('bnd,bme->bnm',x_,x_).mean()
        x_cossim = torch.tril(abs(x_ @ x_.transpose(-2, -1))).sum((-1, -2))/(256*257/2)
        ...
        # ################ COSINE SIMILARITY MEASURE
        # x_norm = torch.norm(output.permute(1,0,2), 2, dim = -1, keepdim= True)
        # x_ = output.permute(1,0,2)/x_norm
        # # x_cossim = torch.einsum('bnd,bme->bnm',x_,x_).mean()
        # x_cossim = torch.tril(x_ @ x_.transpose(-2, -1)).sum((-1, -2))/(256*257/2)
        # x_cossim = x_cossim.mean()
        # print('hihi')
        # import pdb;pdb.set_trace()
        # ################
...
# Standard multihead attention.
class RMultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
                 pre_lnorm=False, layerth = None):
        super(RMultiHeadAttn, self).__init__()
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        ############ R-former
        ### only for self-attention since qlen must = klen 
        src_v = 0.4*(head_v0  - head_v) if head_v0 is not None else 0.
        # import pdb;pdb.set_trace()
        attn_vec = attn_vec + src_v
        ############ 
...
class CudaFastWeightSumTwoLinearTransformerLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0,
        ...
        if self.update_mode in ['rbf', 'rbf2keys']:
            self.pi0 = nn.Parameter(0.5 * torch.ones(self.n_head, 1), requires_grad= True)
            self.pi1 = nn.Parameter(0.5 * torch.ones(self.n_head, 1), requires_grad= True)
...
# Katharopoulos et al's Linear Transformer https://arxiv.org/abs/2006.16236
# = Sum update rule + ELU based attention function
class CudaFastWeightSumLinearHDPLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0,
                 pre_lnorm=False, eps=1e-5, layer_id=None, num_layer=None,
                 skip_attn_normalization=False, n_global_head = None, name_method = None):
        super(CudaFastWeightSumLinearHDPLayer, self).__init__()
        print(f"Using CudaFastWeightSumLinearTransformerLayer {layer_id} -")
        ...
        # (3 * d_head * n_head) for qkv and (1 * n_head) for beta.
        self.q_net = nn.Linear(d_model, n_head * d_head, bias=False)
        self.k_net = nn.Linear(d_model, n_global_head * d_head, bias=False)
        self.v_net = nn.Linear(d_model, n_global_head * d_head, bias=False)

        if name_method in ['hard_fish', 'fish']:
            self.hdp_net = nn.Linear(self.n_global_head, self.n_head, bias=False)
            self.hdp_net_k = nn.Linear(self.n_global_head, self.n_head, bias=False)
        elif name_method in ['hard_gfish', 'gfish']:   
            ...
            self.hdp_net_k = nn.Sequential(
                nn.Linear(self.n_global_head, self.n_head, bias=True),
                nn.ReLU(),
                nn.Linear(self.n_head, self.n_head, bias=True)
            )
        if name_method in ['gfish', 'fish']:
            self.sigma_noise = nn.Parameter(0.1 * torch.ones(1), requires_grad= True)
            self.sigma_noise_k = nn.Parameter(0.1 * torch.ones(1), requires_grad= True)
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        head_q = head_q.view(h.size(0), h.size(1), self.n_head, self.d_head)
        head_k = head_k.view(h.size(0), h.size(1), self.n_global_head, self.d_head)
        head_v = head_v.view(h.size(0), h.size(1), self.n_global_head, self.d_head)
...
#         head_k = F.tanh(head_k) + 1.
        ...
        if self.normalize_attn_scores:
            if self.name_method in ['gfish', 'fish']:
                head_k_global = head_k + (self.sigma_noise_k**1) * torch.randn_like(head_k)
            else:
                head_k_global = head_k
            head_k_global = self.hdp_net_k(head_k_global.permute(0,2,3,1)).permute(0,3,1,2)
            # head_k_global = head_k_global / head_k_global.sum(-1, keepdim=True)
            denominator_acc = torch.cumsum((head_k_global), dim=2)
        ...
        # import pdb;pdb.set_trace()
        assert mems is None
        assert (mem_fast_weights == 0.).all()
        # layer_out = fast_weight_sum(head_q, head_k, head_v, mem_fast_weights)
        kv = torch.einsum('bhund,bhune->bhude', head_k.unsqueeze(-2), head_v.unsqueeze(-2)).cumsum(dim = -3)
        
        # if self.normalize_attn_scores:
        if self.name_method in ['gfish', 'fish']:
            kv = kv + (self.sigma_noise**1) * torch.randn_like(kv)
        kv = self.hdp_net(kv.permute(0,2,3,4,1)).permute(0,4,1,2,3)
        layer_out = torch.einsum('bhund,bhude->bhune', head_q.unsqueeze(-2), kv+mem_fast_weights.unsqueeze(-3)).squeeze(-2)
        # import pdb;pdb.set_trace()
...
class DecoderLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, dropout, attn_type, 
                update_mode = 'rbf', scale_w = 1., n_global_head = None, name_method = None, layerth = None,
                 **kwargs):
        ...
        if attn_type == 2:
            ...
            attn_func = RCudaFastWeightSumLinearTransformerLayer
        elif attn_type == 200:
            attn_func = MGKAttn
        elif attn_type == 400:
            ...
            attn_func = CudaFastWeightSumLinearHDPLayer
        else:
            raise Exception(f"attn_type {attn_type} not allowed here.")

        if attn_type in [400, 500, 200]:
            self.dec_attn = attn_func(n_head, d_model, d_head, dropout, 
            update_mode = update_mode,**kwargs)
        elif attn_type == 700:
            self.dec_attn = attn_func(n_head, d_model, d_head, dropout, 
            n_global_head = n_global_head, name_method = name_method,**kwargs)
        elif attn_type in [222, 111]:
            self.dec_attn = attn_func(n_head, d_model, d_head, dropout, layerth= layerth, **kwargs)
        else:
            self.dec_attn = attn_func(n_head, d_model, d_head, dropout, **kwargs)
        self.pos_ff = PositionwiseFF(
            d_model, d_inner, dropout, pre_lnorm=kwargs.get('pre_lnorm'))
    ...
    def forward(self, *dec_inp, dec_attn_mask=None, mems=None, redraw=True,
                carry_over_fast_weight=False, head_v0 = None):
        if self.attn_type in [222, 111]:
            if self.layerth == 0:
                output, head_v0 = self.dec_attn(*dec_inp, attn_mask=dec_attn_mask, mems=mems, carry_over_fast_weight=carry_over_fast_weight)
            else:
                output = self.dec_attn(*dec_inp, attn_mask=dec_attn_mask, mems=mems, carry_over_fast_weight=carry_over_fast_weight, head_v0 = head_v0)
        ...
        if carry_over_fast_weight:
            assert 1==2
            return output, new_mem
        if self.layerth == 0: return output, head_v0
        else:return output
...
class MemTransformerLM(nn.Module):
    def __init__(self,
        ...
        if attn_type == 0:  # the default attention
        ...
        elif attn_type in [2, 3, 4, 600, 2010]:  # absolute embeddings
            ...
            self.is_r_former = True # absolute embeddings
            # 2: baseline vanilla transformer
            # 3:
            # 4: linear transformer
            for i in range(n_layer):
        ...
        elif attn_type in [10, 14, 24, 34, 44, 400, 500, 700]:  # fast weights
                ...
                self.layers.append(
                    DecoderLayer(
                        ...
                        skip_attn_normalization=skip_attn_normalization,
                         n_global_head = n_global_head, name_method = name_method)
                )

        elif attn_type == 111:  # fast weights
            # 10: debugging, same as linear trafo but step by step
            # 14: linear fast weight
            for i in range(n_layer):
                self.layers.append(
                    DecoderLayer(
                        ...
                        attn_type=attn_type, layer_id=i, num_layer=n_layer,
                        skip_attn_normalization=skip_attn_normalization,
                         n_global_head = n_global_head, name_method = name_method, layerth = i)
                )
        ...
        if self.attn_type == 0:  # default attention
            ...
            self.r_bias = nn.Parameter(torch.Tensor(
                    self.n_layer, self.max_klen, self.n_head))
                                ...
                                24, 25, 26,
                                34, 35,
                                44, 45, 46, 200, 300, 400, 500, 202, 600, 700, 2010,222, 111]:
    ...
    def _forward(self, dec_inp, mems=None, carry_over_fast_weight=False):
        ...
        elif self.attn_type in [2, 4, 5, 6, 7, 10, 14, 16, 200, 202, 600, 2010]:  # absolute
            if self.no_pos:
                core_out = self.drop(word_emb)
            else:
        ...
        ####### R-former
        elif self.attn_type == 222:  # absolute
            if self.no_pos:
                core_out = self.drop(word_emb)
            else:
                pos_seq = torch.arange(klen-1, -1, -1.0,
            ...
            if mems_i is not None and i == 0:
                mems_i += pos_emb[:mlen]
            if self.attn_type == 5:
                core_out, head_v0 = self.layers[0](core_out, dec_attn_mask=dec_attn_mask,
                                    mems=mems_i, redraw=self.training)
            else:
                core_out, head_v0 = self.layers[0](core_out, dec_attn_mask=dec_attn_mask,
                                    mems=mems_i)
            ...
            for i in range(1, len(self.layers)):
                ...
                if mems_i is not None and i == 0:
                    mems_i += pos_emb[:mlen]
                if self.attn_type == 5:
                    core_out = self.layers[i](core_out, dec_attn_mask=dec_attn_mask,
                                     mems=mems_i, redraw=self.training, head_v0 = head_v0)
                else:
                    core_out = self.layers[i](core_out, dec_attn_mask=dec_attn_mask,
                                     mems=mems_i, head_v0 = head_v0)
                hids.append(core_out)
        #######

        elif self.attn_type in [24, 25, 26, 34, 35, 44, 45, 46, 300, 400, 500, 700]:  # absolute
            if self.no_pos:
                core_out = self.drop(word_emb)
            else:
                pos_seq = torch.arange(klen-1, -1, -1.0,
        ...
        ####### R-linear-former
        elif self.attn_type == 111:  # absolute
            if self.no_pos:
                core_out = self.drop(word_emb)
            else:
                pos_seq = torch.arange(klen-1, -1, -1.0,
            ...
            core_out, head_v0 = self.layers[0](core_out, dec_attn_mask=dec_attn_mask, mems=mems_i,
                        carry_over_fast_weight=carry_over_fast_weight)
            hids.append(core_out)
            ########
            ...
            for i in range(1, len(self.layers)):
                ...
                core_out = self.layers[i](core_out, dec_attn_mask=dec_attn_mask, mems=mems_i,
                        carry_over_fast_weight=carry_over_fast_weight, head_v0 = head_v0)
                hids.append(core_out)
